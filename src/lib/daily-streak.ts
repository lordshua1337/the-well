...
const STORAGE_KEY = "the-well-daily-streak";
...
export function loadStreak(): StreakState {
  ...
  try {
    ...
    // Last active yesterday -- streak is alive, just not completed today yet
    if (stored.lastActiveDate === yesterday) {
      return { ...stored, completedToday: false };
    }

    // Last active day before yesterday -- grace period (freeze, don't reset)
    if (stored.lastActiveDate === dayBeforeYesterday) {
      return { ...stored, completedToday: false };
    }

    // More than 2 days missed -- reset streak, keep lifetime stats
    return {
      currentStreak: 0,
...
export function getReachedMilestone(streak: number): number | null {
  // Return the milestone that was just reached, if any
